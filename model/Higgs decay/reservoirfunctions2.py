# -*- coding: utf-8 -*-
"""
Created on Tue Sep 12 10:31:03 2023
...
class Reservoir:
    ...
    def train(self,inps_and_targs, monitor_training=0, **kwargs):
        ...
        *****NOTE***** The function inps_and_targs must have a third output of "hints" for training.
        ...
        DRNN = Reservoir(hyperparameters = self.p, num_inputs = D_num_total_inps, num_outputs = 1)
    ...
    def test(self, inps_and_targs, **kwargs):
        ...
        return E_norm[0],out,targ
    
    
    
    
        
    def rtlpap(self, inps_and_targs, trials, **kwargs):
        p = self.p
        self.initialize_act()
        print('Recurrent Activity initialization',end="")
        for i in range(p['test_init_trials']):
            print('.',end="")
            inp, targ = inps_and_targs(dt=p['dt'], **kwargs)[0:2]
            self.run(inp)
