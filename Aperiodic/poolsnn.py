# -*- coding: utf-8 -*-
"""
Created on Tue Nov  7 14:41:42 2023
...
class Subnetwork(object):
    ...
    def get_incoming_currents(self, t, f_out=None, h=None):
        ...
        if self.f_out:
            # print('t',t)
            # print('f_out[t] shape',f_out[t].shape)
            # print('u_out shape',self.u_out.shape)
            # print('f_out[t]',f_out[t])
            # print('u_out',self.u_out)
            # print('f_out',f_out)
    ...
    def step(self, f, t, f_out=None, h=None):
        ...
        # print('u_in shape',self.u_in.shape)
        # print('f[t]',f[t])
        # print('u_in',self.u_in)
        # print('f',f)
        ...
        print(np.sum(np.matmul(self.w, self.r*self.dt), 0))
...
class S_RNN(object):
    ...
    def __init__(self, N_neurons, N_inputs, input_dims, N_outputs, output_dims, alpha, gg, gp, Qg, Qp, dt, tm_g, tm_p, \
        ...
        start_value = 0.001
        end_value = 0.01
        num_elements = 1000 # You can change this number as needed

        # Create a linearly spaced array of values
        diagonal_elements = np.linspace(start_value, end_value, num_elements)
        # Create a diagonal matrix using the numpy.diag function
        alpha = np.diag(diagonal_elements)
        #alpha = 0.0001
